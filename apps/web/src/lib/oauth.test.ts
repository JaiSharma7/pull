...
describe('OAuth sign-in', () => {
  it('asks Microsoft for the email scope and returns to the original idea', () => {
    const request = oauthRequest('azure', 'https://whatapull.vercel.app', '/pull/123#p-123');
    ...
    expect(redirect.origin).toBe('https://whatapull.vercel.app');
  ...
  it('keeps external and malformed destinations out of the callback', () => {
    for (const next of ['https://evil.test', '//evil.test', '/\\evil.test', '/\nevil.test']) {
      expect(oauthRequest('google', 'https://whatapull.vercel.app', next).options?.redirectTo).toBe(
        'https://whatapull.vercel.app/',
      );
    }
  });
