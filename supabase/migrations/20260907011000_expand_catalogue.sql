-- Queue verified, missing public-domain sources through the existing BAML generation pipeline.
-- No duplicate published works or queued/running jobs; no reader owns these public summaries.
with target(title, kind, url, author) as (values
     ...
     missing as (
       ...
       where not exists (
         select 1 from public.works w where lower(w.title) = lower(t.title)
       )
       and not exists (
