-- The feed. Returns the rows to show, how many known ideas were skipped (which
-- the UI renders as time saved), and where the questions go.
...
begin
  ...
  candidates as (
    ...
    where s.status = 'published'
      ...
      -- Don't re-show what they saw recently.
  ...
  marked as (
    select c.*,
      -- Directly known: they have actually studied this card.
      (uid is not null and exists (
        select 1 from public.knowledge_states ks
        where ks.user_id = uid and ks.pull_id = c.id
          and public.retrievability(ks.stability, ks.last_seen_at) > 0.7
      )) as seen_directly,
      -- Semantically covered: a specific idea they remember is near-identical.
      (c.embedding is not null and exists (
        select 1 from known_ideas ki where (ki.embedding <=> c.embedding) < 0.14
      )) as covered,
      -- Novelty: distance from the nearest thing they already know.
      coalesce((
        select min(ki.embedding <=> c.embedding) from known_ideas ki
    ...
    select m.*,
      (  0.28 * public.topic_affinity(m.work_id, weights)
       + 0.18 * case
                  ...
                  else greatest(0.0, 1.0 - (m.embedding <=> uvec))
                end
       ...
       + 0.08 * m.trust_score
       -- Small deterministic jitter so equal scores do not always tie-break the
       -- same way, without making the order unstable within a session.
       + 0.10 * public.seeded_unit(p_seed, p_page, m.ordinal, 'jitter')
      ) as score
    from marked m
  ),
  surviving as (
    select * from scored where not (seen_directly or covered)
  ),
  -- Diversity: at most two cards from the same work per page, so one prolific
  -- source cannot take over the feed.
  diversified as (
  ...
  select
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'work', jsonb_build_object(
        'id', f.work_id, 'title', f.work_title, 'slug', f.work_slug,
        'kind', f.work_kind, 'year', f.work_year
      ),
  ...
  -- The Delta's headline number: what we did not make them read again.
