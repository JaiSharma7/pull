-- Supersede the pushed Delta migration without rewriting applied history.
-- These changes keep false suppression conservative and relation writes owner-only.
revoke all on public.delta_relations from public, anon, authenticated;
grant select on public.delta_relations to anon, authenticated;
create trigger delta_relations_updated_at
  before update on public.delta_relations
...
comment on constraint delta_relations_approval on public.delta_relations is
  'Elaboration may be proposed but not approved until its direction is modeled.';
...
as $$
  select coalesce((
    select ((re.kind in ('review', 'recall', 'say_it_back')
      ...
      and public.retrievability(coalesce(re.stability_after, ks.stability),
...
comment on function public.delta_has_evidence(uuid) is
  'Latest server-applied retrieval or Already knew it evidence, vetoed by failures applied after submission, above the recall-time floor using attempt stability.';
...
begin
  ...
  with known_candidates as materialized (
    -- Bound history probes before checking evidence. A state beyond this cap
    -- remains visible even if an in-cap state lacks proof.
    ...
    where uid is not null and ks.user_id = uid
      and public.retrievability(ks.stability, ks.last_seen_at)
          > public.known_retrievability_floor()
    order by strength desc, ks.pull_id
    limit public.known_comparison_cap()
  ),
  known_ideas as materialized (
    -- Include null embeddings: reviewed equivalence still works without one.
    select kc.* from known_candidates kc
    where public.delta_has_evidence(kc.pull_id)
  ),
  ...
  pool as (
    ...
    from candidates c
    order by cheap_score desc, c.id
    limit pool_size
  ),
  judged as (
    select pl.*,
  ...
  diversified as (
    select s.*, row_number() over (partition by s.work_id order by s.score desc, s.id) as per_work
    from scored s
    where not s.covered
  ),
  final as (
    select * from diversified where per_work <= 2 order by score desc, id limit p_limit
  ),
  /*
  ...
  rows_json as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'reason', f.reason
    ) order by f.score desc, f.id), '[]'::jsonb) as v
    from reasoned f
  ),
  covered_delta as (
    select count(*) as n, coalesce(sum(estimated_read_seconds), 0) as secs
...
comment on function public.get_feed(int, bigint, int, int, int, int) is
  'Personalised feed. Proven known comparisons cap before history probes; equal-score IDs stabilize pagination.';
...
begin
  ...
  known_ideas as materialized (
    select kc.pull_id, kc.embedding from known_candidates kc
    where public.delta_has_evidence(kc.pull_id)
  ),
...
comment on function public.get_source_delta(uuid) is
  'Work-wide Delta across readable published summaries, with known-state cap before history probes.';

create or replace function public.get_summary_delta(p_summary_id uuid)
returns jsonb
language plpgsql
stable
security invoker
...
begin
  ...
  from public.pulls p
  join public.summaries s on s.id = p.summary_id
  where s.id = p_summary_id and s.status = 'published';
...
revoke all on function public.get_summary_delta(uuid) from public, anon;
grant execute on function public.get_summary_delta(uuid) to authenticated;
comment on function public.get_summary_delta(uuid) is
  'Delta counts only the selected readable published summary displayed by the Source page.';
