...
-- local stack, so revoking it unconditionally makes the whole migration
-- unreplayable — `supabase db reset` dies here on a fresh checkout. Guard it:
-- close the endpoint where the function exists, and stay silent where it does
-- not. Anything we do not create ourselves needs this treatment.
do $$
begin
  if exists (
    select 1
