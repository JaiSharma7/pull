create or replace function public.due_pressure(p_user_id uuid)
returns real
language sql
stable
security invoker
...
create or replace function public.dismissal_damping(p_user_id uuid)
returns real
language sql
stable
security invoker
...
as $$
  ...
  select case
    when (select count(*) from recent) < 3 then 1.0::real
    else greatest(0.25, 1.0 - (
      select count(*)::real from recent where response = 'dismissed'
    ) / greatest((select count(*)::real from recent), 1.0))::real
  end;
$$;
