-- Codex review: the Delta did roughly 2 x C x K 1536-dimensional distance
-- calculations per feed request — two separate correlated subqueries (one for
-- `covered`, one for `novelty_distance`) over every published candidate C
-- against every remembered idea K, before the limit was applied. At 10,000
-- cards and 1,000 remembered ideas that is tens of millions of comparisons on
-- the read path, which is exactly what the cost law exists to prevent.
--
-- Three changes, in order of effect:
--   1. Compute the nearest distance ONCE per candidate in a lateral join and
--      derive both `covered` and `novelty_distance` from it. Halves the work.
--   2. Shortlist candidates on cheap, non-vector signals first, so the vector
--      work runs over a bounded set rather than the whole corpus.
--   3. Cap the remembered set, so K cannot grow without limit as a library does.
--
-- This bounds the cost; a precomputed neighbour table is the real fix and is
-- recorded in docs/roadmap.md as round 2 work.

-- Adding a defaulted parameter creates an overload rather than replacing it.
drop function if exists public.get_feed(int, bigint, int, int, int);
...
declare
  ...
  -- Generous enough not to change results at any realistic page size, tight
  -- enough that the vector work stays bounded.
  shortlist_size constant int := 300;
...
begin
  ...
  with known_ideas as (
    -- Strongest first: if the cap bites it should drop the ideas closest to
    -- being forgotten, not the ones the reader most reliably holds.
    select p2.embedding
  ...
  shortlist as (
    -- Cheap ranking only. No vector maths runs until the set is bounded.
  ...
  measured as (
    -- One distance computation per candidate, reused for both purposes.
  ...
  marked as (
    select m.*,
      (uid is not null and exists (
        ...
        where ks.user_id = uid and ks.pull_id = m.id
          and public.retrievability(ks.stability, ks.last_seen_at) > 0.7
      )) as seen_directly,
...
comment on function public.get_feed(int, bigint, int, int, int, int) is
  'Personalised feed page: rows, Delta skip count, and interleaved question slots. Vector work is bounded by a shortlist. No LLM.';
